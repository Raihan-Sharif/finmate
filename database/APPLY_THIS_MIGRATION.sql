-- =============================================
-- CRITICAL MIGRATION: Admin Functions and RLS Policy Fixes
-- APPLY THIS MANUALLY IN YOUR DATABASE CONSOLE
-- =============================================

-- This migration must be applied with database superuser privileges
-- Copy and paste this entire file into your Supabase SQL editor or database console
...
-- Drop existing restrictive policy that prevents admin access to all coupons
DROP POLICY IF EXISTS "coupons_select_policy" ON coupons;

-- Create new policy that allows admin full access to all coupons
-- Regular users can only see active public coupons or their specific assigned coupons
CREATE POLICY "coupons_admin_full_access" ON coupons
    FOR ALL USING (
        -- Admin role has full access to all coupons (active and inactive)
        EXISTS (
...
-- Function to get all coupons for admin (bypasses RLS completely)
...
-- Function to get all subscription payments for admin (bypasses RLS completely)
...
-- ✅ Admin coupon management functions (CRUD)
...
-- 1. Admins can see and manage all coupons (active/inactive)
-- 2. Regular users only see active public coupons
-- 3. Admin API endpoints will work without RLS errors
-- 4. Payment management will function properly
-- 5. All functions use SECURITY DEFINER to bypass RLS restrictions