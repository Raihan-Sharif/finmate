-- FinMate Database Deployment Script (Fixed)
-- Run this in Supabase SQL Editor to deploy the complete schema

-- =============================================
-- SAFE CLEANUP (Handles non-existing objects gracefully)
-- =============================================
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'on_transaction_change') THEN
        DROP TRIGGER on_transaction_change ON transactions;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_roles_updated_at') THEN
        DROP TRIGGER update_roles_updated_at ON roles;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_permissions_updated_at') THEN
        DROP TRIGGER update_permissions_updated_at ON permissions;
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_budgets_updated_at') THEN
        DROP TRIGGER update_budgets_updated_at ON budgets;
    END IF;
END $$;
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RAISE WARNING 'Error creating default categories for user %: %', p_user_id, SQLERRM;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- =============================================
SELECT 
    'Database schema deployed successfully!' as message,
    COUNT(*) as total_tables
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('roles', 'permissions', 'role_permissions', 'profiles', 'user_permissions', 'categories', 'accounts', 'transactions', 'budgets', 'user_sessions', 'admin_audit_logs');